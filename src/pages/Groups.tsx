...
import { Plus, FolderOpen, Hash, Pencil, Trash2, X, Check } from "lucide-react";
...
export default function Groups() {
  ...
  const { data: tags = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .order("name");
      if (error) return DEMO_TAGS;
      return data.map((t: any) => ({
        id: t.id,
        name: t.name,
        item_count: t.item_tags?.[0]?.count || 0,
      }));
    },
    retry: false,
  });
  ...
  const addGroup = useMutation({
    mutationFn: async (name: string) => {
      ...
      const { error } = await supabase.from("groups").insert({ name, color });
  ...
  return (
    <div className="min-h-screen bottom-nav-safe">
      ...
      <div className="flex gap-2 px-4 pb-3">
        <Input
          placeholder={`New ${tab === "groups" ? "group" : "tag"} name...`}
          value={newName}
        ...
        {tab === "groups"
          ? groups.map((g, i) => (
              <motion.div
                ...
                className="feed-card flex items-center gap-3"
              >
                <div
...
const DEMO_GROUPS: Group[] = [
  { id: "1", name: "Dev Tools", color: "hsl(175, 80%, 50%)", item_count: 12 },
  { id: "2", name: "Articles", color: "hsl(210, 80%, 55%)", item_count: 8 },
  { id: "3", name: "Work", color: "hsl(38, 92%, 55%)", item_count: 5 },
  { id: "4", name: "Research", color: "hsl(280, 70%, 55%)", item_count: 3 },
];
...
const DEMO_TAGS: Tag[] = [
  ...
  { id: "4", name: "typescript", item_count: 6 },
  { id: "5", name: "productivity", item_count: 4 },
];
